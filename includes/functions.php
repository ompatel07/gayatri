...
// CSRF
//
// Every state-changing POST carries a per-session token. Without this,
// yoursite.com could be made to add to a cart, place a COD order or post a
// review on behalf of a logged-in visitor from an attacker's page.
// ---------------------------------------------------------------------------
...
/**
 * Guard a POST handler. $json makes it answer API-style instead of redirecting.
 */
function csrf_guard($json = false) {
    ...
    // Bounce back to wherever they were, not always the cart.
    $_SESSION['error_msg']   = "Your session expired. Please try that again.";
    $_SESSION['admin_error'] = "Your session expired. Please try that again.";
    header('Location: ' . safe_redirect_target($_SERVER['REQUEST_URI'] ?? '', BASE_URL . '/index.php'));
    exit;
}
 ...
 *
 * ?redirect= went straight into a Location header, so
 * /login.php?redirect=https://evil.example could be mailed out as a
 * legitimate-looking link that dumped the user on an attacker's page
 * immediately after they signed in.
 */
function safe_redirect_target($target, $fallback) {
    ...
    // no scheme, no protocol-relative //host, no header injection
